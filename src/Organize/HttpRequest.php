...
class HttpRequest
{
    ...
    public static function sendRequest($url, $method, $body = null, $times = 1, $retries = Config::DEFAULT_MAX_RETRY_TIMES)
    {
        ...
        if ($errorCode) {
            if ($times < $retries) {
                return self::sendRequest($url, $method, $body, ++$times, $retries);
            } else {
                throw new OrganizeExceptions("Connect Error. Please retry later. Error:" . $errorCode . ":" . self::$errorCodeDesc[$errorCode] . curl_error($ch));
            }
        } else {
